package org.onion.web.controller.user;
...
public class UserController extends GenericController<User, String> {
    ...
    @AccessLogger("查询列表")
    @RequestMapping(value = "/ls/{type}", method = RequestMethod.GET)
    @Authorize(action = "R")
    public ResponseMessage listByType(@PathVariable("type") int type, QueryParam param) {
        param.excludes("password");
        param.and("type", type);
        return super.list(param)
                .exclude(User.class, "password", "modules", "userRoles")
