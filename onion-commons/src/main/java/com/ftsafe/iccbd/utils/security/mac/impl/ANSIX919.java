...
public class ANSIX919 extends AbstractMac {
	...
	public byte[] getMac(byte[] src, byte[] tak) throws MacException {
		...
		System.out.println("macTemp="+ ByteUtil.toHexString(macTemp));
		

		Des des2 = new Des(right);
		byte[] temp = des2.decrypt(macTemp);
		System.out.println("temp="+ByteUtil.toHexString(temp));
	...
	public static void main(String[] args) throws MacException {
		byte[] tak = ByteUtil.toBytes("F76213D3FD26CBAD0BC2755DB0D5F810");
		byte[] src = ByteUtil.toBytes("0200722044C020C0909118628882001030600201000000000000050000121218173800022012340210000636628882001030600201D120452010000595004757313030303237383838313532303030303030303839313536D3078ED63706EDBD0230310011220000010000");
		ANSIX919 ansix919 = new ANSIX919();
		byte[] mac = ansix919.getMac(src, tak);
		String macstr = ByteUtil.toHexString(mac);
