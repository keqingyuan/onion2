 ...
 * Regular Config
 */	
ORYX.CONFIG.SERVER_HANDLER_ROOT = 			"workflow";
...
ORYX.CONFIG.TEST_SERVICE_URL = 				ORYX.CONFIG.SERVER_HANDLER_ROOT + "/workflow/";
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/stencilset.json?version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
...
function init() {
    ...
    // Else fetch the model from server and display editor
		...
		var modelUrl = "./workflow/model/" + modelId + "/json";
	  										...
	  										if (ref == "pimg") {
	  											...
	  											if(onclickAttr) {
	  												if(prop && ("" + prop).length > 0) {
	  													onclickAttr.textContent = "window.location = '../workflow/editor?id=" + prop + "_form'";
	  							    	   			} else {
